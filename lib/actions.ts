...
export async function createProduct(productType: ProductType, formData: FormData) {
  ...
  const rawUploadFeeAmount = calculateUploadFee(price, quantity);
  const promoCode = required(formData.get("promoCode"));
  const { applied: promoApplied, discountedFee: uploadFeeAmount } = applyUploadPromoCode(rawUploadFeeAmount, promoCode);
  console.log("[createProduct] promo code entered:", promoCode || "(none)", "| applied:", promoApplied, "| fee:", uploadFeeAmount);
